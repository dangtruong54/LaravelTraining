    ...
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            <form class="form_upload" method="post" action="{{route('post.postUpload')}}" enctype="multipart/form-data">
                ...
                <input type="file" class="form-control file_upload" name="filename[]" multiple required
            ...
            @if(isset($listImage) && count($listImage) > 0)
                <div class="image-wrap">
                    @foreach($listImage as $item)
                        ...
                        <img style="max-width: 200px" class="img-fluid" src="{{('http://img.domain/app') . '/' . $item->filename}}"/>
...
<script>
    $('document').ready(function () {
                    ...
                    if ($.inArray(input.files[element].type, ValidImageTypes) > 0) {
                        if (input.files[element].size > 1048576) {
                            ...
                            reader.onload = function (e) {
                                ...
                                $('#list-image').append('<input type = "hidden" name ="image[]" value= "' + e.target.result + '"/>')
                            };
        ...
        $('.btn-primary').on('click', function () {
            $('form > .file_upload').attr("disabled",true);
        })
    })
</script>
